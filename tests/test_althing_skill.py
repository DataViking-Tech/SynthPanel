"""Guard the Hermes-compatible director skill at skills/althing/SKILL.md.

The top-level ``althing`` skill exists so Hermes Agent (and other
skills-compatible hosts) can route Althing triggers without ad-hoc
...
SKILL_PATH = REPO_ROOT / "skills" / "althing" / "SKILL.md"
...
def test_plugin_manifest_registers_director_skill() -> None:
    ...
    assert "skills/althing/SKILL.md" in manifest["skills"], (
        "skills/althing/SKILL.md missing from .claude-plugin/plugin.json"
    )
