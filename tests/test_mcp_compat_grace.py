...
The shim sits in front of :func:`althing.structured.validate.validate_request`
...
``ALTHING_SCHEMA_MIN="1.1.0"`` flips the shim into hard-reject mode (no
...
from althing.mcp.compat import (
    LEGACY_DECISION_PLACEHOLDER,
    W_DECISION_MISSING,
    apply_legacy_grace,
)
from althing.structured.validate import validate_request
...
def test_v1_0_synthesizes_legacy_decision(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="althing.mcp.compat"):
...
def test_schema_min_1_1_0_disables_synthesis(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    """``ALTHING_SCHEMA_MIN="1.1.0"`` opts the host into v1.1 hard-reject."""
...
def test_schema_min_1_0_0_keeps_synthesis(monkeypatch: pytest.MonkeyPatch) -> None:
    """An explicit ``ALTHING_SCHEMA_MIN="1.0.0"`` is the same as unset."""
    monkeypatch.setenv("ALTHING_SCHEMA_MIN", "1.0.0")
    out = apply_legacy_grace("run_panel", {})
    assert out["decision_being_informed"] == LEGACY_DECISION_PLACEHOLDER
...
def test_schema_min_future_version_disables_synthesis(
    ...
    monkeypatch.setenv("ALTHING_SCHEMA_MIN", "2.0.0")
    out = apply_legacy_grace("run_panel", {})
    assert "decision_being_informed" not in out
