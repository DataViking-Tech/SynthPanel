...
**SynthBench → althing flow is wired.** `AlthingProvider` at `src/synthbench/providers/althing.py:287-400` consumes althing JSON output via two paths: direct API (`althing.llm.client.LLMClient` import + `ThreadPoolExecutor`) and CLI subprocess fallback. Provider reads only `data["rounds"][0]["results"][*].responses[i].response` per panelist per question; metadata fields (cost, usage) are optional.
...
**Question matching is explicit by dataset structure.** SynthBench orchestrates: loads `Question` from the adapter, passes `question.text + question.options` to the provider. No fuzzy matching, no althing→synthbench question-key registry. Integration assumes althing is invoked only by SynthBench's provider.
...
**Synthpanel → SynthBench is one-way.** `althing/pyproject.toml:46-48` declares `[convergence]` extra requiring `synthbench>=0.1`. Lazy import in `src/althing/convergence.py:554-608::load_synthbench_baseline()` — imports inside the function, raises `SynthbenchUnavailableError` if missing. CLI flag `--convergence-baseline DATASET:QUESTION` (sp-yaru) is the current opt-in surface.
...
Comparing althing's 8 bundled instruments to SynthBench's 9 datasets:

**All 8 althing instruments are 100% free-text today.** Every question declares `response_schema: {type: text}`. No Likert, enum, or yes-no. Branching instruments route on post-hoc theme extraction by the synthesizer, not pre-defined options.
...
5. **`extract_schema` path** — `src/althing/structured/` already transforms free-text responses into structured data via tool-use. The last-mile pipeline from free-text → enum/likert → distribution → JSD already exists component-wise; wiring is the question.
...
**Inline-during-run vs post-hoc tension.** (a) `--calibrate-against DATASET:QUESTION` during `panel run` couples calibration to the run and pays live cost; (b) `althing calibrate RESULT.json --against DATASET` as post-hoc subcommand decouples but requires a second invocation. Both have legitimate use cases.
