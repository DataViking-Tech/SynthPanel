...
SynthBench computes distributional comparisons (JSD, Kendall's tau, convergence curves) between althing output and real human survey data. Map the current data flow end-to-end: how does althing output reach SynthBench, what transformations happen on each side, where does the comparison result live, and who reads it.

## Research dimensions

### 1. SynthBench's ingestion of althing output

- How does SynthBench discover a althing panel result today? Is there a provider adapter? A file-watcher? A manual invocation?
- `src/synthbench/providers/althing.py` exists — what does it expose, and what's its call contract?
...
- How does SynthBench match a althing output question to a SynthBench dataset question? By key, text similarity, explicit mapping, manual alignment?
...
- Is there any "question registry" that althing could reference by name to say "this question is the same as GSS's SPKATH"?
...
- What's the output shape of a full SynthBench run against a althing provider — single JSD per question, a table, a leaderboard entry, a published artifact?
...
- Does althing already import synthbench anywhere, or is the provider one-way (synthbench → althing)?
- What's althing's extras mechanism (`pyproject.toml` → `[project.optional-dependencies]`)? Are there precedents for optional extras that pull in a specific sibling package?
- What's SynthBench's current install size / dependency tree? Would requiring it inline affect althing's PyPI install experience?
...
- What does althing currently do with per-question output that could attach a metric? (e.g. `per_model_results[*].results[i]` is the row level; `per_question_synthesis` is the map-reduce per-question summary.)
...
- What CLI subcommands exist today that explicitly reference validation / calibration / ground-truth? (`althing analyze`, `althing panel inspect`, others?)
...
### 7. Dataset coverage vs althing question types

- Which of althing's 8 bundled instruments (product-feedback, market-research, feature-prioritization, pricing-discovery, landing-page-comprehension, churn-diagnosis, name-test, general-survey) have *any* topical overlap with any SynthBench dataset question?
- Conversely, for each SynthBench dataset, what kinds of althing instruments would naturally ask questions that have a human distribution to compare against?
...
- What's the precedent for free-text→categorical extraction in the codebase (the `--extract-schema` flag, `src/althing/structured/*`)? Is that already plumbed into something SynthBench-adjacent?
