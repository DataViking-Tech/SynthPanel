...
When a checkpoint was written by an older althing, this module
brings it up to CURRENT_SCHEMA_VERSION so the rest of the codebase
can assume the latest shape.

Version history
---------------
v1 — althing 0.11.x initial checkpointing (sp-hsk3); no cli_args field.
v2 — althing 0.12.x; cli_args added (sy-ws76); best_model_for flag added.
...
def migrate_v1_to_v2(old: dict[str, Any]) -> dict[str, Any]:
    """Migrate a v1 checkpoint (althing 0.11.x) to v2 schema.
...
def migrate_to_current(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if version > CURRENT_SCHEMA_VERSION:
        raise ValueError(
            f"checkpoint schema_version {version} is newer than this althing "
            f"installation (max supported: v{CURRENT_SCHEMA_VERSION}). "
            f"Upgrade althing to resume this run."
        )
    if version == CURRENT_SCHEMA_VERSION:
        return data
    result = dict(data)
