...
class LadderConfig:
    ...
    user_agent: str = "althing-fetch/1.0 (+https://althing.dev)"
...
def _step_screenshot(url: str, cfg: LadderConfig) -> bytes | None:
    ...
    Playwright is gated behind the ``althing[visual]`` extra; if it
...
def _store(
    ...
    *,
    ...
    if cfg.l1 is not None:
        from althing.fetch.cache import CacheHit
    ...
    if needs_screenshot:
        ...
        if l1_body is not None:
            ...
            if disk is not None:
                ...
                from althing.fetch.perimeter import safe_resolve
