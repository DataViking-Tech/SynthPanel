...
from althing.stats import chi_squared_test
...
# (src/althing/structured/schemas.py) each reserve exactly one field
...
def compute_calibration_jsd(
    ...
    """Inline-calibration JSD between a althing and a SynthBench distribution.
...
def _lead_interpretation_for_uniform_skew(
    ...
    *,
    ...
    comes from :func:`althing.stats.chi_squared_test` (single-row GOF),
...
def load_synthbench_baseline(spec: str) -> dict[str, Any]:
    ...
    the ``althing[convergence]`` extras and import lazily so the core
    ...
    try:
        ...
        raise SynthbenchUnavailableError(
            "synthbench is required for --convergence-baseline. Install it with: pip install 'althing[convergence]'"
        ) from exc
