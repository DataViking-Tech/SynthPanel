...
``~/.althing/registry-cache.json``) and has the shape described in
structure.md §5::
...
OFFLINE_ENV = "ALTHING_REGISTRY_OFFLINE"
REFRESH_ENV = "ALTHING_REGISTRY_REFRESH"
...
def load_registry(
    *,
    ...
    - ``offline`` (flag or ``ALTHING_REGISTRY_OFFLINE=1``) — never
      touch the network. Returns the cached registry if present, else
      an empty ``{"schema_version": 1, "packs": []}``.
    - ``refresh`` (flag or ``ALTHING_REGISTRY_REFRESH=1``) — bypass
    ...
    try:
        ...
        if cached is not None:
            emit(f"althing: registry fetch failed ({exc}); using stale cache from {cached.fetched_at.isoformat()}")
            return cached.registry
        emit(f"althing: registry unavailable ({exc}); returning empty registry")
        return _empty()
