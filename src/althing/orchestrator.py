...
from althing.attachments import filter_attachments
from althing.attachments.filter import count_strata
from althing.conditions import evaluate_condition, normalize_follow_up
from althing.convergence import ConvergenceTracker, extract_categorical_responses
from althing.cost import ZERO_USAGE, CostGate, TokenUsage, UsageTracker, resolve_cost
from althing.fetch.cache import CacheL1, UrlCache
from althing.fetch.lower import AttachmentFetchError, lower_url_blocks
from althing.instrument import END_SENTINEL, Instrument, Round
from althing.llm.client import LLMClient
from althing.llm.models import ContentBlock, InputMessage, TextBlock, URLBlock
...
from althing.response_coercion import coerce_response, is_typed_schema
from althing.routing import route_round
...
def _unpack_extract_schema(
    ...
    :func:`althing._runners.resolve_extract_schema`
...
# honest. Approximated via chars/4 since althing ships no tokenizer.
...
def run_panel_parallel(
    ...
    Args:
        ...
        on_panelist_complete: Optional callback invoked once per panelist the
            ...
            :mod:`althing.checkpoint` to snapshot progress every K
    ...
    # cross-run dedup on top (default root: ~/.althing/cache/url).
...
def _describe_branch(
    ...
    from althing.routing import evaluate_predicate
    ...
    if len(models) >= 2 and questions:
        ...
        try:
            ...
            if multi_model_responses:
                from althing.stats import convergence_report
