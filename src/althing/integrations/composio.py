"""Composio connector for Althing.

Exposes the Althing SDK as a Composio experimental :class:`Toolkit` so
agents built on LangChain, CrewAI, Semantic Kernel, or AutoGen can invoke
Althing actions through Composio's native tool-calling path (no MCP
...
Usage::
    ...
    tools = session.tools()  # includes Althing actions
    # ...hand `tools` to LangChain / CrewAI / etc.

Five actions are registered under the ``ALTHING`` toolkit slug:
...
:func:`althing_toolkit` once per client. The import itself is cheap
...
# :func:`althing_toolkit` (local scope), that resolution cannot find them.
...
# inside :func:`althing_toolkit` — and mypy — resolve to real types.
...
# namespaced automatically as LOCAL_ALTHING_<action> by Composio.
TOOLKIT_SLUG = "ALTHING"
TOOLKIT_NAME = "Althing"
TOOLKIT_DESCRIPTION = (
    ...
    "result. Runs locally via the Althing SDK; requires an LLM API key "
...
class ComposioNotInstalledError(ImportError):
    ...
    Althing alone is cheap. This error carries the exact ``pip install``
...
def _require_composio() -> Any:
    ...
    try:
        ...
        raise ComposioNotInstalledError(
            ...
            "`pip install althing[composio]` (or `pip install composio`) "
            "to register Althing as a Composio toolkit."
        ) from exc
    return composio
...
def althing_toolkit(composio_client: Any) -> Any:
    """Build a Composio :class:`Toolkit` exposing five Althing actions.
    ...
    if experimental is None or not hasattr(experimental, "Toolkit"):
        raise RuntimeError(
            ...
            "custom-tools API, or open an issue against Althing if "
            "this was unexpected."
        )
