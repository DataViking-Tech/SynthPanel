...
from althing.conditions import ConditionError, validate_condition_string
from althing.structured.schemas import is_known_schema
        ...
        if es is not None:
            if isinstance(es, str):
                if not is_known_schema(es):
                    from althing.structured.schemas import SchemaNotFoundError
...
def _validate_system_prompt_template(template_str: str) -> None:
    ...
    if not any(m in template_str for m in _JINJA2_MARKERS):
        return
    try:
        from althing.prompts import compile_jinja2_template
