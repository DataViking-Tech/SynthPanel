...
def setup_logging(verbosity: str | None = None, *, debug_all: bool = False) -> None:
    """Configure logging for the ``althing`` package.

    Args:
        verbosity: One of ``"debug"``, ``"info"``, ``"warning"``.
            When *None*, falls back to ``ALTHING_LOG_LEVEL`` (case-insensitive),
            then ``INFO``. Ignored for level picking when ``debug_all`` is True
            (althing always DEBUG in that mode).
    ...
    if debug_all:
        ...
        env = os.environ.get("ALTHING_LOG_LEVEL", "").strip().lower()
    ...
    logger = logging.getLogger("althing")
