...
* :mod:`althing.attachments.models` — typed records for
  attachment kinds and refs (hq-qd7r).
* :mod:`althing.attachments.store` — content-addressable
  persistence (hq-qd7r). A global CAS at
  ``$SYNTH_PANEL_ATTACHMENT_DIR`` (or
  ``$SYNTH_PANEL_DATA_DIR/attachments``, defaulting to
  ``~/.althing/attachments``) shards files under a 2-char sha256
...
* :mod:`althing.attachments.filter` — per-persona stratification
  (hq-iczd). Decides which attachments each panelist sees based on
  predicate filters against persona traits.
* :mod:`althing.attachments.pdf` — PDF ingest decision tree
...
from althing.attachments.models import AttachmentKind, AttachmentRef
from althing.attachments.pdf import (
    DEFAULT_OPTIONS,
    PdfEncryptedError,
    PdfError,
