...
retry policy lives in :mod:`althing.structured.output`; this module
...
from althing.structured.validate import ErrorEnvelope

logger = logging.getLogger(__name__)

DRIFT_DEGRADE_ENV: str = "ALTHING_DRIFT_DEGRADE"
...
def degrade_enabled(env: Mapping[str, str] | None = None) -> bool:
    """Return True when ``ALTHING_DRIFT_DEGRADE`` selects degraded-artifact mode.
    ...
    if val in _FALSY:
        return False
    logger.debug(
        "ALTHING_DRIFT_DEGRADE=%r is not a recognized truthy/falsy value; falling back to v1.0.0 default (%s).",
        raw,
    ...
    *,
    ...
    return ErrorEnvelope(
        ...
        message=(
            ...
            "artifact is not delivered because ALTHING_DRIFT_DEGRADE "
            "is off (v1.0.0 default). Set ALTHING_DRIFT_DEGRADE=1 to "
