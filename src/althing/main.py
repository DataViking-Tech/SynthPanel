...
from althing.cli.repl import run_repl
from althing.logging_config import setup_logging


def _quiet_broken_pipe() -> None:
    """Make `althing … | head` exit silently like a regular Unix tool.
...
def _main(argv: list[str] | None) -> int:
    ...
    if args.command == "prompt":
        ...
        if sub == "summary":
            ...
            from althing.cli.commands import handle_cost_show
        ...
        from althing.cli.commands import handle_poll_summary
...
def main_deprecated_synthpanel(argv: list[str] | None = None) -> int:
    """Entry point for the legacy ``synthpanel`` console script.

    Kept for one deprecation cycle after the rename to ``althing`` so
    existing shell scripts, CI pipelines, and MCP client configs keep
    working. Prints a notice to stderr, then behaves exactly like
    :func:`main`.
    """
    print(
        "note: the 'synthpanel' command has been renamed to 'althing' — "
