...
Search order for ``--profile NAME``:
  1. Bundled profiles (package data: ``src/althing/packs/profiles/``)
  2. ``./profiles/NAME.yaml`` (working directory)
  3. ``~/.althing/profiles/NAME.yaml`` (user home)
"""
...
def _user_profiles_dir() -> Path:
    ...
    return Path.home() / ".althing" / "profiles"
