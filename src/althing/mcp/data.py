...
Data is stored under ~/.althing/ (configurable via SYNTH_PANEL_DATA_DIR).
...
def _bundled_packs() -> dict[str, dict[str, Any]]:
    """Load persona packs bundled in althing.packs.

    Returns a dict mapping pack_id (filename stem) to parsed YAML data.
    """
    result: dict[str, dict[str, Any]] = {}
    try:
        packs_pkg = _resource_files("althing.packs")
...
def _bundled_instrument_packs() -> dict[str, dict[str, Any]]:
    """Load instrument packs bundled in althing.packs.instruments.

    Returns a dict mapping pack_id (filename stem) to parsed YAML data.
    """
    result: dict[str, dict[str, Any]] = {}
    try:
        pkg = _resource_files("althing.packs.instruments")
...
def list_instrument_packs() -> list[dict[str, Any]]:
    ...
    ``althing.packs.instruments``) and user-saved packs under
...
def get_panel_result(result_id: str, *, load_attachments: bool = False) -> dict[str, Any]:
    ...
    data["id"] = result_id
    if load_attachments:
        from althing.attachments.models import AttachmentRef
        from althing.attachments.store import refs_path
...
def save_panel_synthesis(
    ...
    Used by ``althing panel synthesize`` (sp-5on.5) to persist a
...
def save_panel_result(
    ...
    * ``metadata``: the run provenance bundle produced by
      :func:`althing.metadata.build_metadata` (version, config_hash,
      cost/pricing snapshot, timing, models). Persisted at the top level
      so ``althing report`` can populate its provenance table for
    ...
    * ``attachments``: ``{ref_id: AttachmentRef}`` map written to a
      ...
      live in CAS (see :mod:`althing.attachments.store`); only
      refs land here.
