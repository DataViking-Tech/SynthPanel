...
Sits in front of :func:`althing.structured.validate.validate_request` on
...
see legacy traffic. Setting ``ALTHING_SCHEMA_MIN="1.1.0"`` (or any later
...
_logger = logging.getLogger("althing.mcp.compat")
...
_SCHEMA_MIN_ENV = "ALTHING_SCHEMA_MIN"
...
def apply_legacy_grace(tool: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    The hard-reject path activates when ``ALTHING_SCHEMA_MIN`` is set to
