"""On-disk credential store for Althing (sp-lve).

Users who don't want to export an ``ANTHROPIC_API_KEY`` (etc.) in every
shell can run ``althing login`` to persist a key to
``~/.config/althing/credentials.json`` (mode ``0600``). Provider code
...
Claude Code installed types ``althing prompt`` and hits "Missing API
...
offer a dedicated ``althing login`` instead of auto-bridging.
"""
...
# Recognised provider env var names. ``althing login`` validates
...
# Required prefix(es) for each provider's API keys. ``althing login``
...
# ``althing login --provider <name>`` hint without importing CLI code.
...
def missing_api_key_message(env_var: str, *, alt_env_vars: tuple[str, ...] = ()) -> str:
    ...
    persistent (``althing login --provider <name>``) and one-shot
    ...
    if cli_name:
        msg = (
            ...
            f"or run `althing login --provider {cli_name}` to persist a key on disk."
        )
    else:
        msg = f"Missing API key: set {env_var_list}, or run `althing login` to persist a key on disk."
...
def detect_provider_from_key(value: str) -> str | None:
    ...
    Used by ``althing login`` to suggest the correct ``--provider``
...
def credentials_path() -> Path:
    ...
    Honors ``ALTHING_CREDENTIALS_PATH`` for tests and XDG-style
    overrides, then falls back to ``~/.config/althing/credentials.json``.
    """
    override = os.environ.get("ALTHING_CREDENTIALS_PATH", "").strip()
    if override:
