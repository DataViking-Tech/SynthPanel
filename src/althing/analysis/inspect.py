...
The CLI subcommand ``althing panel inspect <result>`` renders the
report below without making any LLM calls. It works on two result
shapes:

* the flat shape written by :func:`althing.mcp.data.save_panel_result`
...
The report is intentionally independent of :mod:`althing.analyze`
...
from althing.text_width import pad
...
def _collect_model_rollup(
    ...
    when present (it's built in :mod:`althing.metadata`). We fall
