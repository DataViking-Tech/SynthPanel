...
This module keeps the historical ``synthpanel`` import working for one
...
_warnings.warn(
    "'synthpanel' has been renamed to 'althing' — update imports to "
    "'import althing' and install with 'pip install althing'. "
    "The 'synthpanel' alias will be removed in a future major release.",
    DeprecationWarning,
    stacklevel=2,
)
...
for _name, _module in list(_sys.modules.items()):
    if _name == "althing" or _name.startswith("althing."):
        _aname = "synthpanel" + _name[len("althing") :]
...
def __getattr__(name: str):  # PEP 562 — delegate everything else to althing
    ...
    parent-package attributes the way a real import does, so ``synthpanel.sdk``
