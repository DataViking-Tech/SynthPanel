...
class PdfMissingDependencyError(PdfError):
    ...
    Install with ``pip install synthpanel[pdf]``.
    """
...
def _import_pdfium():
    try:
        ...
        raise PdfMissingDependencyError(
            "pypdfium2 is required for PDF attachments. Install with `pip install synthpanel[pdf]`."
        ) from exc
    return pdfium
...
def _import_pillow():
    try:
        ...
        raise PdfMissingDependencyError(
            "Pillow is required to render PDF pages as images. Install with `pip install synthpanel[pdf]`."
        ) from exc
    return Image
