...
oversized prompt 400'd — and SynthPanel classified the 400 as a fatal,
...
def downstream_provider_of(exc: BaseException) -> str | None:
    ...
    :mod:`synth_panel.llm.providers.openrouter` embeds in the message.
    """
    node: BaseException | None = exc
    seen: set[int] = set()
    while node is not None and id(node) not in seen:
...
def format_recovery_command(result_id: str | None, model: str | None) -> str:
    ...
    return f"synthpanel panel synthesize {ref} --synthesis-model {suggest_recovery_model(model)}"
